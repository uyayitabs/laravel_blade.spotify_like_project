    ...
    $.enginePlayMedia = {
        ...
        addSongsToPlayer: function (last, songs, forcePlay) {
            if (songs.data.length) {
                var num = songs.data.length;
                var playerSongsArray = [];
                for (var i = 0; i < num; i++) {
                    var song = $.engineUtils.toPlayerJson(songs.data[i]);
...
(function (factory) {
    ...
    window.Playlist = {
        ...
        playSongsNowOrNext: function (songs, forcePlay) {
            ...
            const canPlaySongRoles = [1, 2, 3, 4, 5, 11];
            let canPlaySong = false;

            if (window.User.userInfo && window.User.userInfo.group && window.User.userInfo.group.role) {
                canPlaySong = canPlaySongRoles.includes(window.User.userInfo.group.role.id);
            }

            if (!canPlaySong) {
                const isViewOnlyUser = (window.User.userInfo.group.role.id === 9);
                const isGuestUser = (window.User.userInfo.group.role.id === 6);

                if (User.isLogged()) {
                    if (isViewOnlyUser || isGuestUser) { // [View Only] user || [Guest] user
                        // PurchaseOnlyFeature
                        if (parseInt(songs[0].selling) && !songs[0].streamable) {
                            $.engineLightBox.show("lightbox-purchaseOnlyFeature");
                            return false;
                        }
                        // VIP Only (Plus Subscription ONLY)
                        if (!parseInt(songs[0].selling) && !songs[0].streamable) {
    ...
    $(document).ready(function () {
        ...
        EMBED.Event.add(window, "embedPlayerEventEndedFired", function () {
            ...
            if (!Playlist.liveRadio) {
                ...
                if (window.User.userInfo && !window.User.userInfo.can_download) {
