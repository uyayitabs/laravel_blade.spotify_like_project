(function (factory) {
    ...
    $(document).on('click', ".lightbox-signup-select .signup-as-listener", function () {
        event.preventDefault();
        if (!$(".lightbox-signup-select").hasClass("hide")) {
            $(".lightbox-signup-select").addClass("hide");
        }
        setTimeout(function() {
            User.SignUp.signupAsListener();
        }, 100);
    });
    ...
    User.SignUp = {
        ...
        signupSelect:  $(".lightbox-signup-select"),
        ...
        captcha: function() {
            grecaptcha.ready(function() {
                grecaptcha.execute($('meta[name="recaptcha-key"]').attr('content'), {action: 'subscribe_newsletter'})
                    .then(function(token) {
        ...
        init: function() {
            $.engineLightBox.show("lightbox-signup-select");
        },
        signupAsListener: function () {
            if (!$(".lightbox-signup-select").hasClass("hide")) {
                $(".lightbox-signup-select").addClass("hide");
            }
            ...
            $('#singup-form').ajaxForm({
                ...
                success: function(response, textStatus, xhr, $form) {
                    ...
                    if ($form.attr('action') === route.route('frontend.auth.info.validate')) {
                        ...
                        // If I'm an artist is not checked, redirect to /settings/subscription
                        if (!$('#im-artist').is(':checked')) {
                            setTimeout(function(){
                                $(window).attr('location', '/settings/subscription');
                            }, 2500);
                        }
            ...
            typingTimer = setTimeout(function () {
                $.ajax({
                    ...
                    success: function () {
                        ...
                        User.SignUp.signuprForm.submit();
                    },
    ...
    User.SignUpArtist = {
        loaded: false,
        finishedButton: $(".lightbox-signup-artist .finished"),
        submitButton: $(".lightbox-signup-artist .submit"),
        error_box:  $(".lightbox-signup-artist .lightbox-error"),
        signuprForm:  $("#signup-artist-form"),
        showError: function(msg){
            this.error_box?.removeClass("hide").html(msg);
        },
        hideError: function(el){
        ...
        reset: function(){
            User.SignUpArtist.signuprForm.trigger("reset");
            this.error_box?.addClass("hide");
            $(".lightbox-signup-artist").find(".control-group").removeClass("field-error");
            $(".lightbox-signup-artist").find(".control-group").removeClass("success");
            $(".lightbox-signup-artist #signup-stage-signup-artist").removeClass("hide");
            $(".lightbox-signup-artist #signup-stage-profile").addClass("hide");
            $(".lightbox-signup-artist #signup-stage-complete").addClass("hide");
            User.SignUpArtist.signuprForm.find("[type='submit']").removeClass("hide");
            User.SignUpArtist.signuprForm.find("[type='submit']").html(Language.text.SIGN_UP);
            User.SignUpArtist.signuprForm.attr('action', route.route('frontend.auth.info.validate'));
        },
        captcha: function() {
            grecaptcha.ready(function() {
                grecaptcha.execute($('meta[name="recaptcha-key"]').attr('content'), {action: 'subscribe_newsletter'})
        ...
        init: function () {
            $('#signup-artist-form').ajaxForm({
                ...
                success: function(response, textStatus, xhr, $form) {
                    User.SignUpArtist.reset();
                    User.SignUpArtist.hideError();
                    User.SignUpArtist.signuprForm.find("[type='submit']").addClass("hide");

                    if (response.activation !== undefined) {
                        $(".lightbox-signup-artist .lb-nav-outer").addClass("hide");
                        $(".lightbox-signup-artist .continue.finished").addClass("hide");
                        $(".lightbox-signup-artist .submit").addClass("hide");
                        $(".lightbox-signup-artist .close").removeClass("hide");
                        $(".lightbox-signup-artist #signup-stage-signup-artist").addClass("hide");
                        $(".lightbox-signup-artist #signup-stage-profile").addClass("hide");
                        $(".lightbox-signup-artist #signup-stage-verify").removeClass("hide");
                    } else {
                        setTimeout(function() {
                            $(".lightbox-signup-select").addClass("hide");
                            User.SignIn.me();
                ...
                error: function(e, textStatus, xhr, $form) {
                    ...
                    User.SignUpArtist.showError(e.responseJSON.errors[Object.keys(e.responseJSON.errors)[0]][0])
                    $form.find("[type='submit']").removeAttr("disabled");
                }
            });
        },
    ...
    // Initialize signup artist form
    User.SignUpArtist.init();
            ...
            $.ajax({
                ...
                success: function () {
                    ...
                    if (window.location.pathname === route.route('frontend.homepage')) {
                        $(window).trigger({ type: "engineReloadCurrentPage" });
                    }
                },
                error: function (jqXHR) {
        ...
        me: function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    const isArtistUser = window.User.userInfo.artist_id > 0;

                    // Conditional check after login
                    switch (window.location.pathname) {
                        case '/sign-in':
                            // Logged-in user is an artist - redirect to /upload page
                            if (isArtistUser && REDIRECT_2_UPLOAD) {
                        ...
                        case '/':
                            if (isArtistUser) {
                                ...
                                // Possible user is [View only] and not yet subscribed.
                                if (User.userInfo.should_subscribe) {
                                    $(window).trigger({ type: 'engineNeedHistoryChange', href: '/settings/subscription' });
                                }
                            }
                            break;
                        default:
    ...
    window.Connect = {
        thirdParty: {
            redirect: function (service) {
                let redirectUrl = route.route('frontend.auth.login.socialite.redirect', {'service': service});
                if (window.location.pathname === '/signup/artist') {
                    redirectUrl = '/connect/redirect/' + service + '/artist';
                }
                window.open(redirectUrl,'_blank','height=450,width=750');
            },
    ...
    const formatUsernameInput = function(value){
        return value.toLowerCase().replace(/[^a-z0-9]/gi, '');
    }

    // Listener signup
    const validateListenerUsernameInput = function(e){
        const usernameInput = $(".lightbox-signup #signup-username");
        ...
        if (username.length > 0) {
            ...
            typingTimer = setTimeout(function () {
                $.ajax({
                    ...
                    success: function () {
                        User.SignUp.hideError();
                        User.SignUp.signuprForm.find("[type='submit']").prop('disabled', false);
                        usernameInput.addClass('text-success');
                        usernameInput.removeClass('text-danger')
                    },
                    error: function (jqXHR) {
                        var serverMsg =  JSON.parse(jqXHR.responseText);
                        User.SignUp.showError(serverMsg.errors[Object.keys(serverMsg.errors)[0]][0]);
                        User.SignUp.signuprForm.find("[type='submit']").prop('disabled', true);
                        usernameInput.addClass('text-danger');
                        usernameInput.removeClass('text-success');
                    }
                })
    ...
    $('.lightbox-signup input[name=name]').on('keyup', function (e){
        const signupName = $('.lightbox-signup input[name=name]').val();
        $('.lightbox-signup input[name=username]').val(formatUsernameInput(signupName));
        $('.lightbox-signup input[name=artist_name]').val(signupName);
        $('.lightbox-signup input[name=username]').trigger('keyup');
    });
    $('.lightbox-signup input#signup-username').on('keyup', validateListenerUsernameInput);
    $('.lightbox-signup input#signup-username').on('focusout', validateListenerUsernameInput);


    // Artist signup
    const validateArtistUsernameInput = function(e){
        const usernameInput = $(".lightbox-signup-artist #signup-username");
        const username = usernameInput.val();
        if (username.length > 0) {
            clearTimeout(typingTimer);
            typingTimer = setTimeout(function () {
                $.ajax({
                    ...
                    success: function () {
                        User.SignUpArtist.hideError();
                        User.SignUpArtist.signuprForm.find("[type='submit']").prop('disabled', false);
                        usernameInput.addClass('text-success');
                        usernameInput.removeClass('text-danger')
                    },
                    error: function (jqXHR) {
                        var serverMsg =  JSON.parse(jqXHR.responseText);
                        User.SignUpArtist.showError(serverMsg.errors[Object.keys(serverMsg.errors)[0]][0]);
                        User.SignUpArtist.signuprForm.find("[type='submit']").prop('disabled', true);
                        usernameInput.addClass('text-danger');
                        usernameInput.removeClass('text-success');
                    }
                })
    ...
    $('.lightbox-signup-artist input[name=name]').on('keyup', function (e){
        const signupName = $('.lightbox-signup-artist input[name=name]').val();
        $('.lightbox-signup-artist input[name=username]').val(formatUsernameInput(signupName));
        $('.lightbox-signup-artist input[name=artist_name]').val(signupName);
    });
    $('.lightbox-signup-artist input[name=artist_name]').on('keyup', function (e){
        const signupName = $('.lightbox-signup-artist input[name=artist_name]').val();
        $('.lightbox-signup-artist input[name=username]').val(formatUsernameInput(signupName));
    });
    $('.lightbox-signup-artist input#signup-username').on('keyup', validateArtistUsernameInput);
    $('.lightbox-signup-artist input#signup-username').on('focusout', validateArtistUsernameInput);
                ...
                update: function(e, ui) {
                    ...
                    $.ajax({
                        ...
                        success: function(response){
                            ...
                            $(window).trigger({ type: "engineSiteContentChange" });
                        }
                    });
                },
                stop: function(e, ui) {
    ...
    $( document ).ready(function() {
        ...
        if (Cookies.get('ASK_USER_TO_SUBSCRIBE') === 'true') {
            Cookies.remove('ASK_USER_TO_SUBSCRIBE');
            $(window).trigger({ type: 'engineNeedHistoryChange', href: '/settings/subscription' });
        }
    });
});
