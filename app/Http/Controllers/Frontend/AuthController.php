...
class AuthController
{
    ...
    public function signupArtist()
    {
        ...
        // Create Artist
        $artist = new Artist();
        $artist->name = $this->request->artist_name;
        $artist->user_id = $user->id;
        $artist->verified = true;
        $artist->save();

        // Save artist_id in $user
        ...
        RoleUser::updateOrCreate(
            ['user_id' => $user->id],
            ['role_id' => Role::getIdByName('Artist')]
        );
        ...
        if (auth()->attempt($login, true)) {
            /** Send Approved Artist email */
            try {
                (new Email())->approvedArtist($this->request);
            } catch (\Exception $e) {
            }
    ...
    public function socialiteLogin(string $provider, string $artist = null)
    {
        if (request()->route()->getName() == 'frontend.auth.login.socialite.redirect' && ! request()->isMethod('post')) {
            if ($artist) {
                \Cookie::queue(\Cookie::make('SOCIALITE_SIGNUP_ARTIST', 'true', 60));
            }
            if (($provider) == 'google') {
                return Socialite::driver('google')->setScopes(['openid', 'email', 'profile'])->redirect(compact('artist'));
            } else {
                return Socialite::driver($provider)->redirect(compact('artist'));
            }
        } else {
            if (($provider) == 'twitter') {
            ...
            if (auth()->check()) {
                ...
                if (isset($service->email)) {
                    ...
                    if ($authUser) {
                        ...
                        if (boolval(\decryptCookie('SOCIALITE_SIGNUP_ARTIST'))) {
                            // Create Artist
                            $artist = new Artist();
                            $artist->name = $authUser->name;
                            $artist->user_id = $authUser->id;
                            $artist->verified = true;
                            $artist->save();

                            // Save artist_id in $user
                            $authUser->artist_id = $artist->id;
                            $authUser->save();
                ...
                $user = User::create([
                    ...
                    'username' => $service->name ? Str::slug($service->name, '') : strtolower(Str::random(16)),
    ...
    {
        Cookie::queue(Cookie::forget('REDIRECT_2_UPLOAD'));
        Cookie::queue(Cookie::forget('SOCIALITE_SIGNUP_ARTIST'));
        auth()->logout();
        return Response::json(array(
            'success' => true,
    ...
    public function settingsProfile()
    {
        ...
        $artist = Artist::findOrFail($user->artist_id);
    ...
    public function managePlaylist()
    {
        ...
        if ($this->request->input('removeIds')) {
            $removeIds = json_decode($this->request->input('removeIds'));
        }
        if ($this->request->input('nextOrder')) {
        ...
        if (is_array($nextOrder) && count($nextOrder)) {
    ...
    public function artistClaim()
    {
        ...
        if ($this->request->input('stage') == 'account') {
        ...
        } elseif ($this->request->input('stage') == 'info') {
            $this->request->validate([
            ...
            if (! isset($row->id)) {
                ...
                $artistRequest->user_id = $authUserId;

                $this->request->whenHas('artist_name', function () use ($artistRequest, $authUserId) {
                    $inputArtistName = $this->request->input('artist_name');
                    $artist  = Artist::where('name', 'like', '%' . $inputArtistName . '%')
                        ->firstOr(
                            function () use ($inputArtistName, $authUserId) {
                                // If no Artist record found, create new one
                    ...
                    $artistRequest->artist_name = $artist->name;
                });

                $artistRequest->phone = $this->request->input('phone');
                $artistRequest->ext = $this->request->input('ext');
                $artistRequest->affiliation = $this->request->input('affiliation') ?? 'Artist/Band Member';
                $artistRequest->message = $this->request->input('message') ?? 'I am the artist';
                ...
                return response()->json([
                    ...
                    'errors' => array('message' => array('You have already submitted a request for an artist profile, please wait for our email.'))
                ], 403);
            }
        }
    }
    ...
    public function cancelSubscription()
    {
        ...
        // Cancel Stripe or Paypal subscription
        try {
            switch (strtolower($subscription->gate)) {
                case 'stripe':
                    $subscription->cancelStripeSubscription();
                    break;
                case 'paypal':
                    $subscription->cancelPaypalSubscription();
                    break;
            }
        } catch (\Exception $exception) {
