...
(function (factory) {
        ...
        disableSubmit: function(e) {
            if ($(this).val().length > 0) {
                ...
                if (parent.hasClass('field-error')) {
                    ...
                    $('.ajax-form').find("[type='submit']").removeAttr("disabled");
                }
            }
            e.preventDefault();
        }
            ...
            error: function (e, textStatus, xhr, $form) {
                ...
                if (e.status === 429) {
                    ...
                    $.each(errors, function (key, value) {
                        ...
                        $form.find("[name=" + key + "]").closest(".control").addClass("field-error");
                        Toast.show("error", value[0], null);
                    });
                    const errorMessage = errorMessages.join('<br>');
                    $form.find('.error').removeClass('hide').html(errorMessage);
                    // $form.find("[name='" + Object.keys(e.responseJSON.errors)[0] + "']").closest(".control").addClass("field-error");
                    $form.find("[type='submit']").attr('disabled', 'disabled');
                }
                $form.find("[type='submit']").removeClass('btn-success');
                $form.find("[type='submit']").removeClass("btn-loading");
            }
        });
        $('.ajax-form input').on('focusout', AjaxForm.disableSubmit);
    };
    ...
    $(document).on('click', '[data-action=dashboard]', function (e) {
        e.preventDefault();
        $(window).trigger({
            type: 'engineNeedHistoryChange',
    ...
    $(document).on('click', '[data-action=home]', function (e) {
        e.preventDefault();
        $(window).trigger({
            type: 'engineNeedHistoryChange',
            href: route.route('frontend.homepage')
