(function($) {
    ...
    window.showGetVerified = function () {
        event.preventDefault();
        setTimeout(() => {
            Artist.claim.init(null);
        }, 200)
    }
    ...
    $.contextMenu.types.subscription = function (item, opt, root) {
        let contextSubscription = '<div class="context-subscription-text">' + Language.text.ENJOY_PREMIUM_FEATURES + '</div><a href="' + route.route('frontend.settings.subscription') + '" class="context-subscription-button">' + Language.text.SUBSCRIBE + '</a>';

        if (User.userInfo.artist_id === 0) {
            contextSubscription += '<a href="#" onclick="window.showGetVerified()" class="context-subscription-button">Become an Artist</a>';
        }

        $('<div class="context-subscription">' + contextSubscription + '</div>')
            .appendTo(this)
            .on('click', 'li', function () {
                // do some funky stuff
                __DEV__ && console.log('Clicked on ' + $(this).text());
    ...
    function ContextMenuAction(key, options) {
        ...
        } else if (key === "upload_songs") {
            window.location.href = route.route('frontend.auth.user.artist.manager.albums.upload', {id: options.$trigger.data('id')});
    ...
    window.contextMenu = {
        ...
        build: function ($trigger) {
            ...
            if (type === 'song') {
                ...
                Object.assign(contextMenu, {
                    label: {type: "label", customName: "Label", disabled: true, className: 'contextmenu-item-info'},
                    "upload_songs": {name: Language.text.UPLOAD_SONGS},
                    "sep0": "---------",
