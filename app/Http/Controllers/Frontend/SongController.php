...
class SongController
{
    ...
    public function index()
    {
        ...
        $view = View::make('song.index')
            ->with('song', $song)
            ->with('sharedBy', auth()->user()->id ?? null)
    ...
    public function download()
    {
        ...
        if ($song->mp3 && $song->allow_download) {
            ...
            } elseif ($this->request->route()->getName() == 'frontend.song.download.hd') {
    ...
    public function downloadOffline()
    {
        ...
        if ($song->mp3) {
            ...
            } elseif ($this->request->route()->getName() == 'api.song.offline.download.hd') {
                if ($song->getFirstMedia('audio')->disk == 's3') {
