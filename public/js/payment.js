    ...
    window.Payment = {
        ...
        init: function (el) {
            ...
            if(el.attr('data-should-hide-description')) {
                ...
                let callbackUrl = route.route('frontend.stripe.purchase.callback')
                if (User.userInfo && User.userInfo.id === 229) {
                    callbackUrl = route.route('frontend.stripe.purchase.callback.v2')
                }
                $('#stripe-form').attr('action', callbackUrl);
