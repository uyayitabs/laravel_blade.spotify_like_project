...
class ArtistAccessController
{
    ...
    public function index()
    {
        ...
        if ($term) {
            $requests = ArtistRequest::withoutGlobalScopes()->where('name', 'like', '%' . $term . '%')->where('approved', false)->paginate(20);
        } else {
            $requests = ArtistRequest::withoutGlobalScopes()->where('approved', false)->paginate(20);
        }

        $total_requests = DB::table('artist_requests')->where('approved', false)->count();
