        ...
        options: {
            ...
            // Callback for successful uploads:
                ...
                if (data.context) {
                    ...
                    alert('Encountered an error uploading your song files(s)');
                }
            },
                ...
                if (data.context) {
                    data.context.each(function (index) {
                        if (data.errorThrown) {
                            ...
                            file.error = data.messages?.uploadedBytesfile?.error || data.errorThrown || data.i18n('unknownError');
                            deferred = that._addFinishedDeferreds();
                            that._transition($(this)).done(
                                function () {
                                    const generic_error = {
                                        'generic_error': [
                                            'There was an error processing your file(s)'
                                        ]
                                    }
                                    var errors = data._response?.jqXHR?.responseJSON?.errors || generic_error
                                    $(this).find('.error').removeClass('hide');
                                    $(this).find('.error').html(errors[Object.keys(errors)][0]);
                                    var that = $(this);
                                    setTimeout(function () {
                                        that.fadeOut();
                                    }, 5000)
