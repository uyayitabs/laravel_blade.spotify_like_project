...
<div id="header-container">
    ...
    <div id="header-user-assets" class="session desktop">
        ...
        <div id="account-buttons" class="user-asset">
            <div class="btn-group no-border-left">
                <a id="settings-button" class="btn" aria-label="Settings" title="Settings">
                ...
                @if (auth()->check() && auth()->user()->group->role_id === 5)
                    <a id="dashboard-button" href="{{ route('frontend.auth.user.artist.manager') }}" class="btn" aria-label="{{ __('web.DASHBOARD') }}" title="{{ __('web.DASHBOARD') }}">
                ...
                @endif
                <a id="upload-button" href="{{ route('frontend.auth.upload') }}" class="btn upload-music hide" aria-label="{{__('web.UPLOAD_SONGS')}}" title="{{__('web.UPLOAD_SONGS')}}">
                    <svg height="29" viewBox="0 0 24 24" width="15" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 0h24v24H0z" fill="none"/>
                        <path d="M9 16h6v-6h4l-7-7-7 7h4zm-4 2h14v2H5z"/>
                    </svg>
                </a>
                <a id="notification-button" class="btn hide" aria-label="{{__('web.NOTIFICATIONS')}}" title="{{__('web.NOTIFICATIONS')}}">
                ...
                @if(config('settings.module_store', true))
                    <a id="cart-button" data-action="show-cart" class="btn hide" aria-label="Cart" title="Cart">
    ...
    <div class="col mobile hide">
        <div id="header-nav-logo"></div>
    </div>
    <div class="col mobile hide">
        <div id="header-nav-title"></div>
    </div>
    <div class="col mobile"> <!-- Home Page -->
        <div id="header-home" data-action="home">
    ...
    @if (auth()->check() && isset(auth()->user()->artist_id) && auth()->user()->artist_id > 0)
    <div class="col mobile"> <!-- Artist Home page -->
        <div id="header-dashboard" data-action="dashboard">
    ...
    @endif
    @if(config('settings.module_store', true))
    <div class="col mobile"> <!-- Cart -->
        <div id="header-cart-menu" data-action="show-cart">
            <span class="header-cart-notification-pill notification-pill hide">
                <span class="notification-count">0</span>
            </span>
    ...
    <div class="col mobile"> <!-- User Profile -->
...
<div id="user-settings-menu" class="mobile">
    <div class="user-section">
        <div class="inner-us">
            ...
            @if (auth()->check() && (!isset(auth()->user()->group->role_id) || auth()->user()->group->role_id == \App\Models\Role::getIdByName('View Only')))
...
<script type="text/javascript">
    ...
    // Custom Cookies
    @if (blank(\decryptCookie('REDIRECT_2_UPLOAD')))
    var REDIRECT_2_UPLOAD=false;
    @else
    var REDIRECT_2_UPLOAD={{\decryptCookie('REDIRECT_2_UPLOAD')}};
    @endif
</script>
...
{{-- COMPILED ASSETS INTO MINIFIED JS FILES WITH CACHE BUSTING --}}
@if(env('COMPILE_ASSETS') === true)
<script src="{{ mix('js/engine.min.js') }}" type="text/javascript"></script>
<script src="{{ mix('js/custom.skin.min.js') }}" type="text/javascript"></script>
<script src="{{ mix('embed/embed.min.js') }}" type="text/javascript"></script>
@else
{{-- NON-COMPILED ASSETS WITH env('APP_VERSION') CACHE BUSTING --}}
