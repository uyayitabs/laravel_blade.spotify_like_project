                ...
                @if($order->user)
                    <tr>
                        ...
                        <td>
                            ...
                            $trialEnd = $order->trial_end ? \Carbon\Carbon::make($order->trial_end)->format('Y-m-d') : null;
                            $nextBillingDate = \Carbon\Carbon::make($order->next_billing_date)->format('Y-m-d');
                            @endphp
                            @if($trialEnd && (\Carbon\Carbon::make($trialEnd)->gt(\Carbon\Carbon::make($now))))
                                <span class="badge badge-info">Trial ends {{ \Carbon\Carbon::make($order->trial_end)->format('F j') }}</span>
                            @elseif(\Carbon\Carbon::make($nextBillingDate)->gte(\Carbon\Carbon::make($now)))
                        ...
                        <td>
                            @if($order->payment_status)
                                <a class="btn btn-danger btn-sm text-white"  href="{{ route('backend.subscriptions.delete', ['id' => $order->id]) }}">Cancel & Refund</a>
                            @else
                                &nbsp;
                            @endif
                        </td>
