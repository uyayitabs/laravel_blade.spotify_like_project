    ...
    function makeActivity($host_id, $object_id, $object_type, $action, $events, $collapse = true, $schedule = null)
    {
        if($schedule) {
            ...
            if($action == "followUser") {
                ...
                $song = Song::withoutGlobalScopes()->findOrFail($object_id);
...
if (!function_exists('decryptCookie')) {
    ...
    {
        return !blank(Cookie::get($cookieName)) ? Cookie::get($cookieName) : null;
    }
}

if (!function_exists('isUserPaypalTester')) {
    function isUserPaypalTester($email)
    {
        $testEmails = explode(',', env('PAYPAL_APP_TEST_EMAILS', []));
        if (count($testEmails)) {
...
if (!function_exists('geoLocationFromIPAddress')) {
    function geoLocationFromIPAddress(): ?string
    {
        $requestIPAddress = request()->ip();
        if (config('app.env') === 'local') {
            $requestIPAddress = '124.104.219.133';
        }
        return \IPLocation::get($requestIPAddress)->countryName ?? '';
    }
}
